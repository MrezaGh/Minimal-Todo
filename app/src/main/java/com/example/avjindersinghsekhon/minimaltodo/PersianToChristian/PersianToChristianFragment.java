...
public class PersianToChristianFragment extends AppDefaultFragment {

    AnalyticsApplication app;
    FloatingActionButton mTransformFloatingActionButton;

    private ArrayList<String> months = new ArrayList<>();
    {
        months.add("Farvardin");
        months.add("Ordibehesht");
        months.add("Khordad");
        months.add("Tir");
        months.add("Mordad");
        months.add("Shahrivar");
        months.add("Mehr");
        months.add("Aban");
        months.add("Azar");
        months.add("Day");
    ...
    private TextView christianDate;
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        year = (EditText) view.findViewById(R.id.yearPersian);
        christianDate = (TextView) view.findViewById(R.id.christianDate);

        mTransformFloatingActionButton = (FloatingActionButton) view.findViewById(R.id.transformFloatingActionButton);
        

        mTransformFloatingActionButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                makeResult(RESULT_OK);// Todo

            }
        });
    }

    private void setDaySpinner(View view) {
        Spinner spin = (Spinner) view.findViewById(R.id.daySpinner);
        ArrayList days = new ArrayList();
        for (int i = 0; i < 30; i++) {
            days.add(i+1);
        }

        ArrayAdapter aa = new ArrayAdapter(getContext(), R.layout.spinner_item, days);
    ...
    private void makeResult(int result) {
        ...
        for (int i = 0; i < months.size(); i++) {
            if (monthSpinner.getSelectedItem().toString().equals(months.get(i))){
                monthNumber = i+1;
            }
        }

        Log.i("year", year.getText().toString());
        Log.i("Month", monthSpinner.getSelectedItem().toString());
        Log.i("Month num ", String.valueOf(monthNumber));
        Log.i("day", daySpinner.getSelectedItem().toString());

        ct.setIranianDate(Integer.parseInt(year.getText().toString()), monthNumber, Integer.parseInt(daySpinner.getSelectedItem().toString()));
        Log.i("Gregorian date ", ct.getGregorianDate());

        christianDate.setText(ct.getGregorianDate());

    }

    private void setMonthsSpinner(View view) {
        Spinner spin = (Spinner) view.findViewById(R.id.monthSpinner);
        ArrayAdapter aa = new ArrayAdapter<>(getContext(),R.layout.spinner_item,this.months);
        aa.setDropDownViewResource(R.layout.spinner_item);
        spin.setAdapter(aa);
    }
    ...
    protected int layoutRes() {
        return R.layout.fragment_persian_to_christian;
    }
    public static PersianToChristianFragment newInstance() {
        return new PersianToChristianFragment();
    }
}
